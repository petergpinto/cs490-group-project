...
class ShowQuestionBank extends Component {
	...
	getRowsData() {
		...
		if(this.props.buildForm) {
			return items.map((row, index)=>{
            	return <tr key={index}>
					<td><button className="append" index={index} onClick={this.handleChange}><PlusButton style={{ 'width': '50px', 'height': 'auto', 'pointer-events':'none'}} /></button></td>
	...
	renderExamQuestions() {
		...
		return (items.map((row, index) => {
			let html = [];
			html.push(<tr><button className="delete" index={index} questionid={row.QuestionId} onClick={this.handleChange}><MinusButton style={{ 'width': '20px', 'height': 'auto', 'pointer-events': 'none' }} /></button></tr>)
